...
class Evaluator():
    ...
    def MI(cls, image_F, image_A, image_B):
        ...
        return skm.mutual_info_score(image_F.flatten(), image_A.flatten()) + skm.mutual_info_score(image_F.flatten(),
                                                                                                   image_B.flatten())
    @classmethod
    def FMI_pixel(cls, image_F, image_A, image_B, bins=256):
     ...
     def compute_mi(x, y):
          ...
          x_discrete = np.uint8(np.round(x_norm * (bins - 1))).flatten()
          y_discrete = np.uint8(np.round(y_norm * (bins - 1))).flatten()
        
          return skm.mutual_info_score(x_discrete, y_discrete)
    
     mi_FA = compute_mi(image_F, image_A)  
     mi_FB = compute_mi(image_F, image_B)  
     mi_AB = compute_mi(image_A, image_B)  
    
     fmi_pixel = (mi_FA + mi_FB) / (2 * mi_AB + 1e-8)
    ...
    def SSIM(cls, image_F, image_A, image_B):
        ...
        if np.issubdtype(image_F.dtype, np.floating):
        ...
        window_size = 3
        ssim_map_AF = ssim(image_F, image_A, data_range=data_range, 
                       full=True, win_size=window_size)[1]
        ssim_map_BF = ssim(image_F, image_B, data_range=data_range, 
                       full=True, win_size=window_size)[1]
        ...
        var_A = local_variance(image_A, window_size)
        var_B = local_variance(image_B, window_size)
        
        
        total_var = var_A + var_B
        
        lambda_A = np.zeros_like(total_var)
        lambda_B = np.zeros_like(total_var)
    
        mask = total_var > 0
        lambda_A[mask] = var_A[mask] / total_var[mask]
        lambda_B[mask] = var_B[mask] / total_var[mask]
    
        zero_mask = ~mask
        lambda_A[zero_mask] = 0.5
        lambda_B[zero_mask] = 0.5

        Q_map = lambda_A * ssim_map_AF + lambda_B * ssim_map_BF
